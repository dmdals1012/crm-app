...
def load_data():
    try:
        ...
        # 'Cluster' → '고객 유형' 컬럼명 변경
        if 'Cluster' in data.columns:
            data = data.rename(columns={'Cluster': '고객 유형'})
        return data
...
customer_type_descriptions = {
    0: {
        "name": "고액 소비 VIP 고객",
        "desc": "이 고객은 고액을 자주 소비하는 VIP 고객입니다.",
        "strategy": "프리미엄 제품 추천, 개인화된 VIP 서비스 제공"
    },
    1: {
        "name": "젊은 신규 고객",
        "desc": "이 고객은 젊은 층으로, 다양한 제품을 시도하는 경향이 있습니다.",
        "strategy": "트렌디한 제품 추천, 소셜 미디어 마케팅 강화"
    },
    2: {
        "name": "중간 소비 젊은 고객",
        "desc": "이 고객은 젊은 층이지만 중간 정도의 구매력을 가지고 있습니다.",
        "strategy": "가성비 좋은 중급 제품 추천, 로열티 프로그램 참여 유도"
    },
    3: {
        "name": "중년층 충성 고객",
        "desc": "이 고객은 중년층으로, 브랜드에 대한 높은 충성도를 보입니다.",
        "strategy": "장기 고객 혜택 강화, 신제품 우선 체험 기회 제공"
    },
    4: {
        "name": "보수적인 중장년층 고객",
        "desc": "이 고객은 중장년층으로, 익숙한 제품을 선호하고 변화를 꺼립니다.",
        "strategy": "신뢰성 강조, 기존 제품의 개선점 홍보"
    },
    5: {
        "name": "가격에 민감한 중년층 고객",
        "desc": "이 고객은 중년층이지만 가격에 민감하며 할인을 선호합니다.",
        "strategy": "할인 행사 정보 우선 제공, 가격 대비 품질 강조"
    }
}

def get_customer_type_name(idx):
    return customer_type_descriptions.get(idx, {}).get('name', f'유형 {idx}')

def get_customer_type_desc(idx):
    info = customer_type_descriptions.get(idx, {})
    return info.get('desc', ''), info.get('strategy', '')
...
def analyze_cluster_purchase(data):
    ...
    desc, strategy = get_customer_type_desc(highest_idx)
    st.markdown(f"- 평균 구매 금액이 가장 높은 고객 유형은 **{highest_name}**이며, 평균 구매 금액은 **{highest_amount:,.2f} USD** 입니다. 👑")
...
def analyze_cluster_rating(data):
    ...
    desc, strategy = get_customer_type_desc(highest_idx)
    st.markdown(f"- 평균 리뷰 평점이 가장 높은 고객 유형은 **{highest_name}**이며, 평균 평점은 **{highest_rating:.2f}** 입니다. 👍")
    st.markdown(f"- **특성:** {desc}")
    st.markdown(f"- **마케팅 전략:** {strategy}")
...
def analyze_cluster_sales(data):
    ...
    desc, strategy = get_customer_type_desc(highest_idx)
    st.markdown(f"- 가장 높은 총 매출액을 기록한 고객 유형은 **{highest_name}**이며, 총 매출액은 **{highest_amount:,.0f} USD** 입니다. 💰")
    st.markdown(f"- **특성:** {desc}")
    st.markdown(f"- **마케팅 전략:** {strategy}")
...
def analyze_cluster_age_distribution(data):
    ...
    type_age = data.groupby(['고객 유형', 'Age Group']).size().unstack(fill_value=0)
    ...
    for idx in type_age.index:
        ...
        desc, strategy = get_customer_type_desc(idx)
        st.markdown(f"- **{get_customer_type_name(idx)}**: 가장 많은 연령대는 **{top_age_group}**이며, 해당 유형의 **{percentage:.1f}%**를 차지합니다.")
        st.markdown(f"  - **특성:** {desc}")
        st.markdown(f"  - **마케팅 전략:** {strategy}")

def describe_customer_types():
    st.subheader("고객 유형별 특성 및 마케팅 전략")
    for idx, info in customer_type_descriptions.items():
        st.markdown(f"### {info['name']}")
        st.markdown(f"- **특성:** {info['desc']}")
        st.markdown(f"- **추천 마케팅 전략:** {info['strategy']}\n")
